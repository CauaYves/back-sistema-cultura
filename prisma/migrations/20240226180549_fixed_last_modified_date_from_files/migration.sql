/*
  Warnings:

  - You are about to drop the column `lasModifiedDate` on the `Files` table. All the data in the column will be lost.
  - Added the required column `lastModifiedDate` to the `Files` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Files" DROP COLUMN "lasModifiedDate",
ADD COLUMN     "lastModifiedDate" TIMESTAMP(3) NOT NULL;
